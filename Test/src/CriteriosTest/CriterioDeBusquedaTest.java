...
class CriteriosDeBusquedaTest {
    ...
    void testCriterioPorDisponibilidad() {
        ...
        when(productoMock.getStock()).thenReturn(15.0);
        assertTrue(criterio.cumple(productoMock));

        // Caso verdadero (justo en el límite)
        when(productoMock.getStock()).thenReturn(10.0);
        assertTrue(criterio.cumple(productoMock));

        // Caso falso
        when(productoMock.getStock()).thenReturn(9.9);
        assertFalse(criterio.cumple(productoMock));
    }
